...
import { getCommerce, updateCommerce } from "@/src/controller";
...
const UpdatingCommerceScreen = () => {
  ...
  useEffect(() => {
    if (user?.userId) {
      getCommerce(user?.userId.toString()).then((res) => {
        console.log(res);
        setForm((prevForm) => ({ ...prevForm, ...res[0] }));
      });
    }
  }, []);
  ...
  return (
    <View style={styles.root}>
      <Stack.Screen options={{ headerTitle: "Atualizar informações" }} />
      <ScrollView
...
export default UpdatingCommerceScreen;
